...
export default function EventCard({ event }: Props) {
  const image =
    (event as Event & { image_url?: string }).image_url || event.image;
  const ageGroup =
    (event as Event & { age_group?: string }).age_group || event.ageGroup;
  const description =
    event.description ||
    (event as Event & { short_description?: string }).short_description ||
    "";
